## ycx_fia_hybrid_fullseries_agb.R
## Biomass (agb_dry) twin of ycx_fia_hybrid_fullseries_vec.R. Same vectorized
## hybrid + agedist/ceiling recalibration engine, but driven by the agb_tonac hybrid
## fits and a DRYBIO_AG growth kernel + ceiling, so above-ground dry biomass is
## projected on the same form as carbon. Everything is kept in Mg/ha so the kernel,
## ceiling, and density stay unit-consistent; the injector rescales to the deployed
## agb_dry t0, so the A0 anchor (reused from the carbon TG file) cancels out.
## Mortality: GRM carries only carbon mortality, so biomass mortality is derived from
## the carbon m(C) grid via a constant carbon fraction CFRAC (drag = m(C*CFRAC)/CFRAC).
## Output: treemap/recal_cell/fia_hybrid_fullseries_agb.csv (state,scenario,year,value,lo,hi)
## Usage: Rscript ycx_fia_hybrid_fullseries_agb.R [out_dir] [fia_tgagc.csv] [GRM_PLT]
...
offs<-seq(0,100,10); TM_BASE<-2025L; LBAC_TO_MGHA<-0.00045359237*2.4710538; TONAC_TO_MGHA<-0.90718474*2.4710538
BIN<-10L; MIN_CELL<-50L; MIN_FT<-80L; MIN_ST<-200L; REGEN_AGE<-5L; HOR<-100L; CFRAC<-0.49
...
## ---- g_obs kernel + ceiling on DRYBIO_AG (Mg/ha) ----
rm<-read.csv(file.path(fia,"plot_remeas.csv"),colClasses="character"); rm$REMPER<-suppressWarnings(as.numeric(rm$REMPER))
agb_of<-function(fips){fp<-file.path(fia,sprintf("%d_TREE.csv",fips)); if(!file.exists(fp))return(NULL)
  hdr<-gsub('"','',strsplit(readLines(fp,1),",")[[1]]); idx<-match(c("PLT_CN","STATUSCD","DRYBIO_AG","TPA_UNADJ"),hdr)
  tmp<-tempfile(fileext=".csv"); system(sprintf("cut -d, -f%s '%s' > '%s'",paste(idx,collapse=","),fp,tmp)); t<-read.csv(tmp); unlink(tmp)
  t$STATUSCD<-suppressWarnings(as.integer(t$STATUSCD)); for(c0 in c("DRYBIO_AG","TPA_UNADJ"))t[[c0]]<-suppressWarnings(as.numeric(t[[c0]]))
  t<-t[!is.na(t$STATUSCD)&t$STATUSCD==1,]; a<-aggregate(I(DRYBIO_AG*TPA_UNADJ)~PLT_CN,t,sum,na.rm=TRUE); names(a)[2]<-"agb"; a$PLT_CN<-as.character(a$PLT_CN); a}
incr<-list()
for(st in names(ABBR2FIPS)){mf<-file.path(cfg,sprintf("ycx_membership_%s.csv",st)); if(!file.exists(mf))next
  ...
  ac<-agb_of(ABBR2FIPS[[st]]); if(is.null(ac))next; ci<-setNames(ac$agb,ac$PLT_CN)
...
INC<-do.call(rbind,incr); pc("biomass kernel data: %d remeas rows\n",nrow(INC))
...
## disturbance params (raster) + GRM carbon m(C) (converted to biomass via CFRAC at use)
...
## hybrid fits: agb_tonac response
H<-list();for(st in names(ABBR2FIPS)){fp<-file.path(out,sprintf("ycx_%s_hybrid_fits.csv",st)); if(!file.exists(fp))next
  f<-read.csv(fp,stringsAsFactors=FALSE); if("response"%in%names(f))f<-f[f$response=="agb_tonac",]
...
C0<-pmax(hyb(a0,Av,kv,pv,dv,Asv)*TONAC_TO_MGHA,0)
Hregen<-pmax(hyb(REGEN_AGE,Av,kv,pv,dv,Asv)*TONAC_TO_MGHA,0)
...
project<-function(rg,arm){
  ...
  for(t in 1:HOR){ age<-age+1
    ...
    gob<-GG[cbind(gid,ai)]
    hin<-hyb(age,Av,kv,pv,dv,Asv)*TONAC_TO_MGHA - hyb(age-1,Av,kv,pv,dv,Asv)*TONAC_TO_MGHA
    w<-pmax(0,pmin(1,(Asv-age)/Asv)); ginc<-pmax(w*gob+(1-w)*hin,0)
    drag<-switch(arm,
      base=0,
      distmod=2*pdistv*sevv*C,
      distsev=3*pdistv*sevv*C,
      mort15=0.5*MG[cbind(mid,pmin(pmax(round(C*CFRAC),0),CMAX)+1L)]/CFRAC,
      mort20=1.0*MG[cbind(mid,pmin(pmax(round(C*CFRAC),0),CMAX)+1L)]/CFRAC)
...
## ---- A0 anchoring (reused carbon TG; cancels in injector t0 rescale) ----
...
pc("A0(placeholder): %d states, median %.0f\n",sum(!is.na(A0)),A0med)
...
res<-do.call(rbind,rows); write.csv(res,file.path(rd,"fia_hybrid_fullseries_agb.csv"),row.names=FALSE)
rr<-aggregate(value~scenario+year,res,sum); r0<-rr[rr$year==2025,]; r100<-rr[rr$year==2125,]
cat("[agb] CONUS dry-biomass hybrid+recal by scenario (t0 -> t100, % shape):\n")
for(bk in names(BUCK)){a<-r0$value[r0$scenario==bk]; b<-r100$value[r100$scenario==bk]; cat(sprintf("  %-46s %+6.1f%%\n",bk,100*(b/a-1)))}
pc("wrote fia_hybrid_fullseries_agb.csv (%d rows)\n",nrow(res))
